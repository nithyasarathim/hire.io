...
const Login = () => {
    ...
    const handleGoogleSuccess = async (tokenResponse) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || 'Student SSO login failed. Ensure the email is registered.');
        }
    };
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-md w-full space-y-8 p-10 bg-white rounded-xl shadow-lg">
                ...
                <form className="mt-8 space-y-6" onSubmit={handleTraditionalSubmit}>
                    <div>
                        <label htmlFor="role-toggle" className="sr-only">Select Role</label>
                        <select
                            id="role-toggle"
                            name="role-toggle"
                            value={roleToggle}
